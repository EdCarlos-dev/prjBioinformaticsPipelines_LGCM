...
def process_one_sample(cram_file, bed_file, output_dir, intermediate_dir,
    ...
    try:
        ...
        logging.error(f"Erro ao converter CRAM para BAM para a amostra {sample_name}: {e}")
        raise  # Relevanta a exceção para que o pipeline principal possa tratar
...
def main(cram_dir, bed_file, output_dir, intermediate_dir,
    ...
    for cram_file in cram_files:
        try:
            ...
            # Decide se continua ou interrompe o pipeline em caso de erro em uma amostra
            # Aqui, estou interrompendo, mas você pode querer continuar com outras amostras
            raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Erro durante a execução do pipeline: {e}") # Exibe o erro para o usuário
        exit(1) # Encerra o script com código de erro diferente de zero
